...
async function streamOllamaCompletion(prompt){
    ...
    // ✅ Everything below should be INSIDE the function!
    if(statusCode !== 200){
        await body.dump();
        throw new Error(`Ollama request failed with status code ${statusCode}`)  // ✅ Fixed: Error instead of GPUInternalError
    }
...
try{
    await streamOllamaCompletion('What is recursion? Explain  ');
}catch(error){
